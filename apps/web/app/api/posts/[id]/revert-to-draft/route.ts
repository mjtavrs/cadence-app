...
export async function POST(_: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const url = new URL(`posts/${encodeURIComponent(id)}/revert-to-draft`, env.apiBaseUrl);
  url.searchParams.set("workspaceId", workspaceId);

  const res = await fetch(url, { method: "POST", headers: { authorization: `Bearer ${accessToken}` } });
  const result = await res.json().catch(() => null);
  return NextResponse.json(result ?? { message: "Falha ao mover post para rascunho." }, { status: res.status });
}

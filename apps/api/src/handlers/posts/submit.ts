...
export const handler: APIGatewayProxyHandler = async (event) => {
  ...
  try {
    ...
    if (!canWrite(membership.role)) return unauthorized("Sem permissão para enviar posts.");
    ...
    await ddb.send(
      new UpdateCommand({
        ...
        UpdateExpression: "SET #status = :review, #updatedAt = :now",
        ExpressionAttributeNames: { "#status": "status", "#updatedAt": "updatedAt" },
        ExpressionAttributeValues: { ":draft": "DRAFT", ":review": "IN_REVIEW", ":now": now },
      }),
    );
    ...
    if (name === "ConditionalCheckFailedException") {
      return badRequest("Post não está em DRAFT.");
    }
    if (name === "NotAuthorizedException") return unauthorized("Sessão expirada. Faça login novamente.");
    return serverError("Não foi possível enviar o post agora.");
  }
};

...
export const handler: APIGatewayProxyHandler = async (event) => {
  ...
  try {
    ...
    if (!canWrite(membership.role)) return unauthorized("Sem permissão para cancelar agendamentos.");
    ...
    await ddb.send(
      new UpdateCommand({
        ...
        UpdateExpression: "SET #status = :approved, #updatedAt = :now REMOVE scheduledAtUtc, weekBucket, GSI2PK, GSI2SK, GSI3PK, GSI3SK",

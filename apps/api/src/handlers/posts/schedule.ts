...
const ALLOWED_SCHEDULE_STATUSES = ["DRAFT", "IN_REVIEW", "APPROVED", "SCHEDULED"] as const;
...
export const handler: APIGatewayProxyHandler = async (event) => {
  ...
  try {
    ...
    if (!ALLOWED_SCHEDULE_STATUSES.includes(status as (typeof ALLOWED_SCHEDULE_STATUSES)[number])) {
      return badRequest("Post não pode ser agendado neste estado.");
    }

    const onlyCalendar = status === "DRAFT" || status === "IN_REVIEW";
    ...
    if (onlyCalendar) {
      await ddb.send(
        new UpdateCommand({
          ...
          UpdateExpression:
            "SET #scheduledAtUtc = :scheduledAtUtc, #weekBucket = :weekBucket, #monthBucket = :monthBucket, #gsi2pk = :gsi2pk, #gsi2sk = :gsi2sk, #gsi4pk = :gsi4pk, #gsi4sk = :gsi4sk, #updatedAt = :now",
      ...
      await ddb.send(
        new UpdateCommand({
          TableName: tableName,
          Key: key,
          ConditionExpression: "#status = :approved OR #status = :scheduled",
          UpdateExpression:
